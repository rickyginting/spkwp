...
class Bobot{
	...
	function readAll(){
		...
		$query = "SELECT * FROM ".$this->table_name." a, wp_kriteria b where a.id_kriteria=b.id_kriteria ";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
	...
	function readAll4(){
		...
		$query = "SELECT * FROM ".$this->table_name." a, wp_kriteria b where a.id_kriteria=b.id_kriteria order by vektor_v DESC";
		$stmt = $this->conn->prepare( $query );
		$stmt->execute();
